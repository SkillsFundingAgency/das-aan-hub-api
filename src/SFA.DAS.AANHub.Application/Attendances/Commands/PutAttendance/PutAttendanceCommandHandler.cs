...
public class PutAttendanceCommandHandler : IRequestHandler<PutAttendanceCommand, ValidatedResponse<SuccessCommandResult>>
{
    ...
    private static string GetTemplateName(PutAttendanceCommand command, Member member)
    {
        ...
        {
            case "Apprentice":
                if (command.IsAttending) { templateName = EmailTemplateName.ApprenticeEventSignUpTemplate; } else { templateName = EmailTemplateName.ApprenticeEventCancelTemplate; }
                break;
            case "Employer":
                if (command.IsAttending) { templateName = EmailTemplateName.EmployerEventSignUpTemplate; } else { templateName = EmailTemplateName.EmployerEventCancelTemplate; }
                break;
        }
