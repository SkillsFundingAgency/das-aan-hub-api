...
public class WhenValidatingFirstName
{
    ...
    public async Task ThenShouldHaveValidValue(int? stringLength, bool isValid, string? errorMessage)
    {
        ...
        target.PatchDoc.Replace(f => f.FirstName, value);
        ...
        else
        {
            result
                ...
                .WithErrorMessage(
                    ...
                    ? string.Format(PatchMemberCommandValidator.ValueIsRequiredErrorMessage, MemberPatchFields.FirstName)
                    : string.Format(PatchMemberCommandValidator.ExceededAllowableLengthErrorMessage, MemberPatchFields.FirstName, 200));
        }
    }
}
