...
public class RequestedByMemberIdValidatorTests
{
    ...
    public async Task Validate_RequestedByMemberId_Exists(Guid? requestedByMemberId, bool isValid)
    {
        var command = new RequestedByMemberIdCommandTest() { RequestedByMemberId = requestedByMemberId };

        var membersReadRepositoryMock = new Mock<IMembersReadRepository>();
        var member = isValid ? new Member() { Status = MembershipStatus.Live } : null;
        membersReadRepositoryMock.Setup(m => m.GetMember(requestedByMemberId)).ReturnsAsync(member);
